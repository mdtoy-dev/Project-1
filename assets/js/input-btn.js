...
// apiKey = 'IZ3Jet89Cn5Z6VkL7lFYgkqB7TJ4XM9h' old key
// new key
apiKey = 'EnUqUrPp9HkUrlhDUAzU5jjzC8SsNnXu'
        ...
        for (var i = 0; i < alphabet.length; i++) {
            ...
            for (var j = 0; j < data.response.countries.length; j++) {
                ...
                if (getLetter === getCountry) {
                    $(`#country-${alphabet[i]}`).append($('<button>').addClass('country-btn').attr({'data-bs-dismiss': 'modal'})
                    .html(`${`<img src="https://flagcdn.com/${(data.response.countries[j]['iso-3166']).toLowerCase()}.svg" width="24" alt="${data.response.countries[j].country_name}">`} ${data.response.countries[j].country_name}`)
                    )
                }
            }
        }
